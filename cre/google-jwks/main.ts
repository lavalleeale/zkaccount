...
import { type Address, encodeAbiParameters, sha256, toHex } from "viem";
import { z } from "zod";
...
function keyCommitment(key: GoogleJwk): `0x${string}` {
  ...
  const digest = sha256(
    toHex(concat([KEY_DOMAIN, lengthPrefix(modulus), modulus, lengthPrefix(exponent), exponent])),
  );
  return `0x00${digest.slice(4)}`;
}

function fetchKeySet(sendRequester: HTTPSendRequester, config: Config): string {
  const response = sendRequester.sendRequest({ url: config.googleJwksUrl, method: "GET" }).result();
  if (!ok(response)) {
...
function onCronTrigger(runtime: Runtime<Config>) {
  ...
  const reportPayload = encodeAbiParameters([{ name: "keys", type: "bytes32[]" }], [keys]);
