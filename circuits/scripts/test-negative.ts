...
function generate(variant?: string): string {
  const result = spawnSync(tsx, variant ? [generator, variant] : [generator], {
    cwd: root,
    encoding: "utf8",
  });
...
executeShouldFail(
  "subject",
  mutateArray(valid, "subject", (values) => {
    values[0] = values[0] === '"0x31"' ? '"0x32"' : '"0x31"';
  }),
);
executeShouldFail(
  "audience",
  mutateArray(valid, "audience", (values) => {
    values[0] = values[0] === '"0x66"' ? '"0x67"' : '"0x66"';
  }),
);
executeShouldFail(
  "nonce",
  mutateArray(valid, "login_randomness", (values) => {
    values[0] = '"0xff"';
  }),
);
executeShouldFail(
  "signature",
  mutateArray(valid, "signature_limbs", (values) => {
    values[0] = `"0x${(BigInt(values[0].slice(1, -1)) + 1n).toString(16)}"`;
  }),
);
executeShouldFail(
  "google_key",
  mutateArray(valid, "modulus_bytes", (values) => {
    values[0] = `"0x${(BigInt(values[0].slice(1, -1)) ^ 1n).toString(16)}"`;
  }),
);
executeShouldFail(
  "issuer",
  mutateArray(valid, "payload", (values) => {
...
executeShouldFail(
  "public_identity",
  mutateArray(valid, "return", (values) => {
    values[0] = `"0x${(BigInt(values[0].slice(1, -1)) + 1n).toString(16)}"`;
  }),
);
executeShouldFail(
  "public_iat",
  mutateArray(valid, "return", (values) => {
    values[7] = `"0x${(BigInt(values[7].slice(1, -1)) + 1n).toString(16)}"`;
  }),
);
